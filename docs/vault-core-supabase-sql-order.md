...
| 10 | `docs/meta-campaign-drafts-schema.sql` | 9.5 | Required for live | `meta_campaign_drafts` (internal Meta campaign PLAN DRAFT artifacts) + enum CHECKs (status/campaign_type/risk_level/target_system pinned to `meta`), a unique index on `source_action_id` (+ indexes on client/agent/profile/status/type/created_at, RLS on, no policies). DRAFT-ONLY — no Meta adapter, nothing launched, no budget changed. Includes an OPTIONAL idempotent backfill (commented-out / manual) that re-lanes legacy `draft_meta_campaign` actions (`content`→`meta`, L2→L3). | Campaign drafts serve mock; created/approved drafts don't persist |
| 11 | `docs/finance-drafts-schema.sql` | 9.6 | Required for live | `finance_drafts` (internal finance / invoice PLAN DRAFT artifacts) + enum CHECKs (status/finance_type/risk_level/target_system ∈ internal/stripe/report), unique indexes on `source_action_id` and `source_snapshot_id` (+ indexes on client/agent/action/snapshot/status/type/created_at, RLS on, no policies). DRAFT-ONLY — no finance adapter: no Stripe invoice created/sent/finalized, no charge, no payment collected, no money moved. `source_snapshot_id` is a safe aggregate ref (`text`). NO backfill / no mutation of existing tables. | Finance drafts serve mock; created/approved drafts don't persist |
| 12 | `docs/creative-briefs-schema.sql` | 9.7 | Required for live | `creative_briefs` (internal content/creative PLAN DRAFT artifacts) + enum CHECKs (status/brief_type/risk_level/target_system ∈ internal/content/social/meta/website), a unique index on `source_action_id` and an expression unique index on `(source_meta_campaign_draft_id, brief_type)` (+ indexes on client/agent/action/campaign/competitor/status/type/platform/created_at, RLS on, no policies). DRAFT-ONLY — no content adapter: nothing posted, published, uploaded, scheduled, or launched. `source_competitor_profile_id` is `text` (may be non-UUID). NO backfill / no mutation of existing tables. | Creative briefs serve mock; created/approved briefs don't persist |
| 13 | `docs/client-health-schema.sql` | 9.8 | Required for live | `client_health_drafts` (internal client-health / retention-risk PLAN DRAFT artifacts) + enum CHECKs (status/health_type/risk_level/target_system ∈ internal/report/ghl/message), unique indexes on `source_action_id`, `source_message_draft_id`, `source_finance_draft_id`, and `(source_snapshot_id, health_type)` (+ indexes on client/agent/action/message/finance/snapshot/status/type/created_at, RLS on, no policies). DRAFT-ONLY — no client-success adapter: no client contacted, no SMS/email sent, no GHL/Stripe/Meta mutation, no workflow trigger. `source_snapshot_id` is a safe aggregate ref (`text`). NO backfill / no mutation of existing tables. | Client health drafts serve mock; created/approved drafts don't persist |

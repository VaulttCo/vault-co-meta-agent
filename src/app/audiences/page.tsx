  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Custom Audiences", count: "4", icon: Target, color: "#f07820" },
          { label: "Lookalike Audiences", count: "2", icon: Users, color: "#18b8f0" },
          { label: "Saved Audiences", count: "1", icon: RefreshCw, color: "#22c55e" },
        ].map((item) => (
          <div key={item.label} className="bg-[#0c0f15] border border-[#1c2438] rounded-xl p-4 flex items-center gap-4 hover:border-[#263050] transition-colors">
            ...
            <div>
              <div className="text-xl font-bold text-[#eef1f8] tracking-tight">{item.count}</div>
              <div className="text-xs text-[#5a6278]">{item.label}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          className="w-full pl-8 pr-3 py-2 bg-[#0c0f15] border border-[#1c2438] rounded-lg text-sm text-[#eef1f8] placeholder-[#5a6278] focus:outline-none focus:border-[#18b8f0]/40 transition-colors"
        />
      </div>
      ...
      <div className="bg-[#0c0f15] border border-[#1c2438] rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#1c2438]">
              {["Audience Name", "Type", "Est. Size", "Status", "Used In", "Overlap"].map((h) => (
                <th key={h} className={`px-5 py-3.5 text-[10px] font-semibold text-[#3d4460] uppercase tracking-widest ${h === "Audience Name" || h === "Type" ? "text-left" : "text-right"}`}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {audiences.map((a, i) => (
              <tr key={a.name} className={`border-b border-[#1c2438]/60 hover:bg-[#131720]/60 transition-colors ${i === audiences.length - 1 ? "border-b-0" : ""}`}>
                <td className="px-5 py-4">
                  <div className="flex items-center gap-2.5">
                    <div className="w-7 h-7 rounded-lg bg-[#131720] border border-[#1c2438] flex items-center justify-center flex-shrink-0">
                      <Users size={12} className="text-[#5a6278]" />
                    </div>
                    <span className="text-[#eef1f8] font-medium">{a.name}</span>
                  </div>
                </td>
                <td className="px-5 py-4"><Badge label={a.type} variant={typeMap[a.type]} /></td>
                <td className="px-5 py-4 text-right text-[#eef1f8]">{a.size}</td>
                <td className="px-5 py-4 text-right"><Badge label={a.status} variant={statusMap[a.status]} /></td>
                <td className="px-5 py-4 text-right text-[#5a6278]">{a.campaigns} campaign{a.campaigns !== 1 ? "s" : ""}</td>
                <td className="px-5 py-4 text-right text-[#5a6278]">{a.overlap}</td>
              </tr>
            ))}
          </tbody>
        </table>

// Vault Core — VICTORIA, Marketing Director (Layer 2). Activated in Phase 3.
//
// Mission: understand how attention converts. Victoria reads available creative/
...
// Obsidian vault. Victoria's discoveries are written to Vault Memory (the source
// of truth), structured so they can be captured into Obsidian via the /victoria
...
const META = getAgentMeta("victoria")!;
...
async function enrich(d: Discovery): Promise<{ headline: string; summary: string } | null> {
  ...
  try {
    const res = await callAnthropicTool<{ headline: string; summary: string }>({
      ...
      system:
        "You are Victoria, Vault Co's Marketing Director. Sharpen one creative/content insight into a crisp headline and a 1–2 sentence summary. Be specific and non-hyperbolic. Never invent numbers not provided.",
...
async function run(ctx: AgentRunContext): Promise<AgentRunResult> {
  ...
  if (await insertActivity({
    agent: META.id,
    kind: "insight",
    message: `Victoria analyzed ${d.campaignCount} campaigns and surfaced: ${label}.`,
    node_id: hookId,
  })) activityCreated += 1;
  ...
  if (d.recommendation) {
    ...
    if (recId) {
      ...
      if (await insertActivity({ agent: META.id, kind: "recommendation", message: `Victoria generated a creative recommendation: ${d.headline}.`, node_id: recId })) activityCreated += 1;
    }
  }
  ...
  if (d.strong) {
    const collabId = await createCollaboration({
      ...
      status: "open",
      summary: `Victoria surfaced a winning creative angle and requested Vega quantify the cross-account impact. ${label}.`,
      related_node_ids: hookId ? [hookId] : [],
    });
    if (collabId) {
...
export const victoriaAgent: RunnableAgent = { meta: META, run };
